...
/**
 * This is the model class for table "tm_menu".
 *
 * @property int $id
...
class Menu extends \yii\db\ActiveRecord
{
    ...
    public function  getAllMenu(){
        $menu = Yii::$app->db->createCommand("SELECT * FROM `tm_menu` WHERE parent='0' ")->queryAll();
        return $menu;
    }
    //获取所有菜单列表
    public function  getMenuList(){
        $menu = Yii::$app->db->createCommand("SELECT * FROM `tm_menu` ORDER BY sort ASC ")->queryAll();
    ...
    public function getRouteById($id){
        $router = Yii::$app->db->createCommand("SELECT * FROM `tm_menu` WHERE id='$id'")->queryOne();
        return $router['route'];
    }
